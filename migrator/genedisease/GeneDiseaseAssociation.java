package grakn.biograkn.migrator.genedisease;
...
public class GeneDiseaseAssociation {
    ...
    private static void migrateFromDisgenet(GraknClient.Session session) {
        try {
            BufferedReader reader = Files.newBufferedReader(Paths.get("dataset/disgenet/curated_gene_disease_associations.csv"));
    ...
    private static void migrateFromClinvar(GraknClient.Session session) {
        try {
            BufferedReader reader = Files.newBufferedReader(Paths.get("dataset/clinvar/gene_condition_source_id.csv"));
            CSVParser csvParser = new CSVParser(reader, CSVFormat.DEFAULT);
