...
class GCPClient:
    ...
    def ssh_session(self, instance):
        ...
        attempts = SSH_MAX_CONNECT_ATTEMPTS
        while attempts:
            _client = paramiko.client.SSHClient()
            _client.set_missing_host_key_policy(paramiko.AutoAddPolicy())

            try:
                _client.connect(instance.public_ip, username='circleci', pkey=self._key)
                self._ssh_client_cache[instance] = _client
                return _client
            except paramiko.ssh_exception.NoValidConnectionsError:
                attempts -= 1
                logger.debug('Could not connect; will attempt %s more times', attempts)
                time.sleep(15)
        raise Exception('Could not connect after %s attempts', SSH_MAX_CONNECT_ATTEMPTS)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.debug('Creating deployment [%s]', deployment_name)
        client.create_deployment(deployment_name)

        instances = client.instances_in_deployment(deployment_name)
        ...
        for i, instance in enumerate(instances, start=1):
            ...
            sp.check_call([
                'bazel',
                'run',
                '@graknlabs_build_tools//ci:run-bazel-rbe',
                '--',
                'bazel',
                'run',
                '--',
        ...
        logger.debug('Removing deployment [%s]', deployment_name)
        if client:
